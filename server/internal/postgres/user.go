...
func (d UserDAO) Load(id uuid.UUID) (*internal.User, error) {
	row := d.Querier.QueryRow(`SELECT name, mail, hash, algorithmID FROM "user" WHERE id=$1`, id.String())
	return d.toUser(id.String(), row)
}

func (d UserDAO) LoadByMail(mail internal.Mail) (*internal.User, error) {
	row := d.Querier.QueryRow(`SELECT id, name, mail, hash, algorithmID FROM "user" WHERE mail=$1`, string(mail))
...
func (d UserDAO) toUser(queryID string, row *sql.Row) (*internal.User, error) {
	...
	if err := row.Scan(&idStr, &name, &mail, &hash, &algorithmID); nil != err {
